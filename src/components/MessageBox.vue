<template>
  <section v-if="isButtonMessageActive" class="fixed inset-x-0 top-20 mx-auto justify-center h-2/3 w-2/3 bg-white rounded-lg hover:shadow-2xl shadow-md border overflow-hidden">

    <button @click="BtnCloseMessage" name="button close message" class="absolute group top-0 w-full h-10 bg-gradient-to-t from-gray-300 to-gray-100">
      <span class="iconify absolute top-2 right-2 text-2xl group-hover:text-gray-500 text-gray-600" data-icon="akar-icons:circle-x-fill"></span>
    </button>
